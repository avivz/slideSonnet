...
- Integration tests marked with `@pytest.mark.integration` (export/render in `test_export_integration.py`, rasterize in `test_pdf_reader.py`, GUI-with-Piper in `test_gui.py`). Run the unit tier with `-m "not integration"`.
...
- External tool dependencies: ffmpeg, ffprobe, pdftoppm, piper (latexmk/pdflatex only to compile demo decks).
